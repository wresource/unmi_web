...
### 重写抢注模块：纯 DropCatch API 数据源

#### 架构重写
- 完全移除 RDAP 扫描逻辑，数据源改为 DropCatch.com API 独占
- 移除候选域名生成器（2-5字母排列组合）
- 新增 `fetchDropDomains()` 替代 `fetchRealDropDomains()`
- 合并 API 拍卖数据和 CSV 下载数据，拍卖数据优先（含真实出价）

#### 三种拍卖类型
- Dropped（已删除） — 已过期释放的域名拍卖
- PrivateSeller（私人卖家） — 私人出售的域名
- PreRelease（预释放） — 即将释放的域名预拍

#### 页面完全重写
- 移除表格视图，改为移动端优先的卡片网格布局
- 点击域名卡片弹出 WHOIS 信息弹窗
- 结束时间精确到分钟（HH:mm），从系统时间实时计算
- 倒计时显示（Xd Xh / Xh Xm / Xm），含紧急程度颜色
- 移除关注列表 Tab 和右侧统计面板
- 水平紧凑筛选栏：状态/长度/TLD/时间/搜索

#### API 未配置提示
- 未设置 API 密钥时显示全页"不可用"状态
- 引导用户前往 DropCatch 申请 API 密钥

#### i18n
- 新增 8 个翻译键（中英双语）：私人卖家、预释放、已删除、已结束、API未配置等
...
#### 纯字母域名扫描
- 仅扫描纯字母域名（a-z），排除数字和连字符
- 域名长度限制 1-5 字符
- 主要后缀：.com、.net、.org
- 重点关注 4 字母和 5 字母 .com 域名
- 候选列表扩展至 500+ 个（全部 2 字母组合、3 字母 CVC、常用 4/5 字母英语单词）

#### 每日自动更新
- 首个访客自动触发数据刷新，后续访客使用缓存数据
- 移除手动"刷新数据"按钮，改为自动机制
- 新增 `needsRefresh()` / `markRefreshed()` 每日缓存逻辑
- 防止并发刷新的互斥锁

#### RDAP 查询优化
- 请求间隔从 500ms 降至 300ms
- 每个 TLD 最多 100 个结果
- 10 天内到期 = 值得抢注
- 提前停止：所有 TLD 达到上限后跳出循环

#### 拍卖监控
- 新增"拍卖监控"Tab，展示有价值的已注册短域名
- 4 字母及以下 .com 域名若已注册，自动标记为 `source = 'auction'`
- 状态显示"监控中"

#### 页面改进
- 新增"今日到期"、"10天内"、"30天内"快速筛选按钮
- 新增 1-5 字母长度快速筛选按钮
- 移除数字/连字符筛选选项（仅纯字母）
- 默认按删除日期升序排序（最快到期的在前）
- 显示"数据每日自动更新"及上次更新时间
